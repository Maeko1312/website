...
module.exports = function (ctx) {
  ...
  for (const f of content.featured.active) {
    ...
    const modules = {
      ...
      useCases: () => html`<div class="fokus-cards">${f.useCases.map(u => html`<article class="fokus-card"><span class="kicker">${u.kicker}</span><h3>${u.h}</h3><p>${u.text}</p></article>`)}</div>`,
      patents: () => html`<div class="fokus-cards is-patents">${f.patents.map((p, i) => html`<article class="fokus-card"><span class="kicker">Patent ${i + 1} · erteilt ${p.granted}</span><h3>${p.h}</h3><p>${p.text}</p><code class="fokus-patent-no" translate="no">${p.no}</code></article>`)}</div>`,
      team: () => html`<div class="fokus-team">${f.team.map(m => html`<article class="fokus-person">${m.image ? html`<img src="${m.image}" alt="${m.name}" loading="lazy" width="160" height="120">` : html`<span class="avatar is-lg" aria-hidden="true">${m.name.split(' ').map(s => s[0]).join('')}</span>`}<div><h3>${m.name}</h3><span class="kicker">${m.role}</span><p>${m.text}</p></div></article>`)}</div>`,
      ...
      valuation: () => html`<section class="fokus-valuation" aria-labelledby="h-val"><h2 id="h-val">Bewertung im Verhältnis</h2><p class="small muted">Rechnerische Größen aus dem aktuellen Kurs (${layout.asOfLabel}) und den zuletzt berichteten Zahlen. Keine Bewertungsempfehlung.</p><div class="number-tiles">${[['Marktkapitalisierung', mcapLive != null ? fmtMio(mcapLive) : f.shareInfo.mcapRef, `${f.shareInfo.sharesLabel} Aktien × Kurs`], ['Kurs / wiederkehrender Umsatz', arrMultiple != null ? num(arrMultiple, 1) + ' ×' : '–', `Börsenwert zu ARR (${f.shareInfo.arrLabel})`], ['Liquide Mittel je Aktie', cashPerShare != null ? num(cashPerShare, 2) + ' ' + inst.currency : '–', '3,78 Mio. CAD zum 30. Juni 2026'], ['Größter Aktionär', f.shareInfo.majorHolder[0], f.shareInfo.majorHolder[1]]].map(([l, v, s]) => html`<div class="number-tile"><span>${l}</span><strong>${v}</strong><small>${s}</small></div>`)}</div></section>`,
  ...
  <div class="layout">
    ...
    <aside class="fokus-ticker card" aria-labelledby="h-ticker">
      ...
      <dl class="kv is-compact fokus-kv">
        ${inst.listings.map(([ex, sym]) => html`<div><dt>${ex}</dt><dd>${copy(sym)}</dd></div>`)}
    ...
    pages.push({ path: url, html: layout.page({ title: `${f.company}: ${f.title}`, description: f.lead, path: url, body, section: null, ogType: 'article', jsonLd, reading: true }) });
  }
  return pages;
};
