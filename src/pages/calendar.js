...
module.exports = function (ctx) {
  ...
  // ---------- IPOs ----------
  {
    const ipoRow = (i) => html`<div class="ipo-row"><span class="when" translate="no">${i.status === 'geplant' ? dateWeekday(D(i.date)) : dateShort(D(i.date))}</span><div><div class="name"><a href="/ipo/${i.slug}">${i.name}</a></div><div class="meta">${i.sector} · ${i.market} · ${i.status === 'geplant' ? 'Preisspanne ' + i.priceRange : i.priceRange + ', Erstkurs ' + i.firstPrice}</div></div><span class="badge ${i.status === 'geplant' ? 'is-accent' : ''}">${i.status === 'geplant' ? 'geplant' : 'gelistet'}</span></div>`;
    const body = html`<div class="container page">
