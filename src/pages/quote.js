  ...
  for (const inst of instruments.all) {
    ...
    const body = html`<div class="container page">
      ...
      <div class="quote-hero">
        <div><span class="kicker" style="color:rgba(255,255,255,.7)">${c.typeLabel(inst)}${inst.type === 'stock' ? ` · ${inst.index} · ${inst.sector}` : inst.region ? ` · ${inst.region}` : ''}</span><h1>${inst.name}</h1><div class="ids" translate="no">${x.isin || inst.isin ? html`<span>${x.isin || inst.isin}</span>` : ''}${util.wkn(x.isin || inst.isin) ? html`<span>WKN ${util.wkn(x.isin || inst.isin)}</span>` : ''}${inst.short && inst.short !== inst.name ? html`<span>${inst.short}</span>` : ''}<span>${inst.exchange || 'Interbanken'}</span><span>${util.time(new Date(config.quotesAsOf))} Uhr</span></div></div>
        ...
        <div class="quote-actions">${c.watchButton(inst.slug)}<button class="btn btn-ghost" type="button" data-share>Teilen</button>${inst.type === 'stock' ? html`<a class="btn btn-ghost" href="/termine/unternehmen#${inst.slug}">Termine</a>` : ''}<a class="btn btn-ghost" href="/analysen">Analysen</a><span class="blurb">${inst.blurb}</span></div>
      </div>
