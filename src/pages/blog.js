    ...
    const body = html`<div class="container page">
      ...
      <div class="layout">
        <article class="article" data-article="${p.title}" data-article-cat="Ratgeber">
          ...
          <figure class="article-hero">${c.thumb('blog-' + p.slug, { image: p.image, label: p.topicObj.name })}${p.imageCredit ? html`<figcaption class="img-credit">Foto: ${p.imageCredit}</figcaption>` : ''}</figure>
