...
module.exports = function (ctx) {
  ...
  c.todayItem = (a) => { const inst = instOf(a); const cat = catOf(a); return html`<li><a href="${c.articleUrl(a)}" aria-hidden="true" tabindex="-1">${c.thumb(a.slug, { label: inst ? inst.short || inst.name : cat.name })}</a><div>${c.storyTop(a)}<h3 class="story-title"><a href="${c.articleUrl(a)}">${a.title}</a></h3></div></li>`; };
  ...
  c.nlClosing = () => { const id = `nl-${++nlCounter}`; return html`<section class="nl-closing" aria-labelledby="nl-closing-title"><div class="container nl-closing-inner"><div class="nl-closing-text"><span class="kicker">Newsletter · kostenlos</span><h2 id="nl-closing-title">Börsenblick am Morgen</h2><p>Jeden Handelstag um 7:30&nbsp;Uhr: die Lage an den Märkten, drei Termine des Tages und die wichtigste Meldung – kostenlos, in zwei Minuten gelesen. Abmeldung jederzeit mit einem Klick.</p></div><div class="nl-closing-form">${nlForm({ id, cls: 'btn-primary', fine: false })}</div></div></section>`; };
