...
module.exports = function (ctx) {
  ...
  c.newsRow = (a) => { const isPost = !!a.topicObj; const inst = isPost ? null : instOf(a); const cat = isPost ? null : catOf(a); const url = isPost ? c.blogUrl(a) : c.articleUrl(a); const label = isPost ? a.topicObj.name : cat.name; const tagUrl = isPost ? c.topicUrl(a.topicObj) : c.catUrl(cat); return html`<article class="news-row ${a.featured ? 'is-promoted' : ''}"><a class="news-row-media" href="${url}" aria-hidden="true" tabindex="-1">${c.thumb((isPost ? 'blog-' : '') + a.slug, { label: inst ? inst.short || inst.name : label, image: a.image })}</a><div><div class="story-top">${a.featured ? html`<span class="badge is-accent">Im Fokus</span>` : ''}<time datetime="${a.date.toISOString()}" translate="no">${relDate(a.date, now)}</time><span class="tag"><a href="${tagUrl}">${label}</a></span></div><h3 class="story-title"><a href="${url}">${a.title}</a></h3><p class="story-excerpt">${isPost ? a.lead : a.deck}</p></div></article>`; };
