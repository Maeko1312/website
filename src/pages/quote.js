    ...
    const body = html`<div class="container page">
      ...
      <div class="layout" style="margin-top:24px">
        <div class="stack">
          ...
          ${extra}
          ${news.length || analyses.length ? html`<section class="grid-2">${news.length ? html`<div class="card">${c.sectionTitle('Nachrichten', { href: '/nachrichten', more: 'Alle' })}${c.storyList(news, { variant: 'is-compact' })}</div>` : ''}${analyses.length ? html`<div class="card">${c.sectionTitle('Analysen', { href: '/nachrichten/analysen', more: 'Alle' })}${c.analysisList(analyses)}</div>` : ''}</section>` : ''}
