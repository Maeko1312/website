...
module.exports = function (ctx) {
  ...
  c.newsRow = (a) => { const isPost = !!a.topicObj; const isGuide = !isPost && !a.categoryObj; const inst = isPost || isGuide ? null : instOf(a); const cat = isPost || isGuide ? null : catOf(a); const url = isPost ? c.blogUrl(a) : isGuide ? `/wissen/${a.slug}` : c.articleUrl(a); const label = isPost ? a.topicObj.name : isGuide ? (a.kicker || 'Wissen') : cat.name; const tagUrl = isPost ? c.topicUrl(a.topicObj) : isGuide ? '/wissen' : c.catUrl(cat); return html`<article class="news-row ${a.featured ? 'is-promoted' : ''}"><a class="news-row-media" href="${url}" aria-hidden="true" tabindex="-1">${c.thumb((isPost ? 'blog-' : isGuide ? 'guide-' : '') + a.slug, { label: inst ? inst.short || inst.name : label, image: a.image })}</a><div><div class="story-top">${a.featured ? html`<span class="badge is-accent">Im Fokus</span>` : ''}${a.date ? html`<time datetime="${a.date.toISOString()}" translate="no">${relDate(a.date, now)}</time>` : html`<span>Ratgeber</span>`}<span class="tag"><a href="${tagUrl}">${label}</a></span></div><h3 class="story-title"><a href="${url}">${a.title}</a></h3><p class="story-excerpt">${isPost ? a.lead : a.deck}</p></div></article>`; };
  // „Begriff des Tages“: ein Lexikon-Begriff je Tag (deterministisch nach Datum), plus drei weitere
  c.termOfDay = () => {
    const g = ctx.content.glossary; if (!g || !g.length) return '';
    const day = Math.floor((now - new Date(now.getFullYear(), 0, 0)) / 86400000);
    const i = day % g.length; const t = g[i];
  ...
  c.nlClosing = () => { const id = `nl-${++nlCounter}`; return html`<div class="container"><section class="nl-closing" aria-labelledby="nl-closing-title"><div class="nl-closing-inner"><div class="nl-closing-text"><span class="kicker">Newsletter · kostenlos</span><h2 id="nl-closing-title">Börsenblick am Morgen</h2><p>Jeden Handelstag um 7:30&nbsp;Uhr in Ihrem Postfach – kostenlos, in zwei Minuten gelesen.</p><ul class="nl-closing-list"><li>Die Lage an den Märkten vor Börsenstart: Asien, US-Futures, DAX-Ausblick</li><li>Die drei Termine, die heute zählen</li><li>Die eine Meldung, die Sie kennen sollten – ohne Produktwerbung</li></ul></div><div class="nl-closing-form">${nlForm({ id, cls: 'btn-primary', fine: true })}</div></div></section></div>`; };
  ...
  c.quizBox = ({ wide = false, half = false } = {}) => {
    ...
    if (wide) return html`<section class="card quiz quiz-wide" aria-labelledby="h-quiz" data-quiz="${z.id}" data-quiz-questions="${JSON.stringify(z.today)}">
      <div class="quiz-intro"><span class="kicker">Börsen-Quiz</span><h2 class="quiz-title" id="h-quiz">Fünf Fragen, täglich neu</h2><p class="quiz-lead">Testen Sie Ihr Börsenwissen: jeden Tag fünf neue Fragen zu Kennzahlen, Märkten und Anlageprodukten – mit kurzer Erklärung zu jeder Antwort.</p><p class="quiz-progress mono" data-quiz-progress>Frage 1 von ${z.today.length}</p><div class="quiz-steps" aria-hidden="true">${z.today.map(() => html`<span data-quiz-seg></span>`)}</div><p class="small muted quiz-note">Ihr Ergebnis bleibt nur in Ihrem Browser.</p></div>
