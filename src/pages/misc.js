...
module.exports = function (ctx) {
  ...
  // ---------- Suche ----------
  {
    const body = html`<div class="container page" data-search-page>
      ...
      <aside>${c.sideCard('Häufig gesucht', html`<div class="chips">${['dax', 'gold', 'bitcoin', 'eur-usd', 'brent', 'silber', 'bund-10j', 'uran'].map(s => { const i = instruments.bySlug[s]; return html`<a class="chip" href="${c.url(i)}">${i.name}</a>`; })}<a class="chip" href="/wissen/boersenlexikon">Börsenlexikon</a></div>`)}${c.sideKnowledge()}</aside></div></div>`;
  ...
  prosePage('/ueber-uns', 'Über Börsenblick', `${config.brand} ist ein unabhängiges Finanzportal für Anlegerinnen und Anleger im deutschsprachigen Raum. Wir erklären, was an den Märkten passiert – in klarer Sprache, mit echten Zahlen und ohne Produktverkauf.`, [
    ...
    { h: 'Wer dahintersteht', html: `<p>Die Beiträge erscheinen unter der <a href="/redaktion">Börsenblick-Redaktion</a>; wie sie entstehen, steht dort. Betreiber ist ${owner(L.company)}, ${owner(L.city)}. Wie wir uns finanzieren, steht unter <a href="/werben">Werben</a>.</p>` },
  ]);

  // ---------- Redaktion: ehrliche Beschreibung, keine erfundenen Profile ----------
  {
    const body = html`<div class="container page">
      ${c.breadcrumb([['Redaktion', '/redaktion']])}
      ${c.pageHead({ kicker: 'Über uns', title: 'Redaktion', lead: html`Wer hinter den Beiträgen steht, wie sie entstehen und wie Sie Korrekturen melden.` })}
      <div class="layout no-sticky"><div class="prose card">
        <h2>Wer schreibt</h2>
        <p>Alle Beiträge auf ${config.brand} erscheinen unter dem Namen <strong>Börsenblick-Redaktion</strong>. Es gibt keine Einzelautoren mit eigenen Profilen; Verantwortlich im Sinne des Presserechts ist ${owner('Name des Verantwortlichen – vom Betreiber zu ergänzen')}. Betreiber und Anschrift stehen im <a href="/impressum">Impressum</a>.</p>
        <h2>Wie Beiträge entstehen</h2>
        <ul>
          <li><strong>Nachrichten und Ratgeber</strong> werden aus öffentlichen Primärquellen recherchiert – Statistikämter, Notenbanken, Unternehmensmitteilungen, Fachmedien – und in eigenen Worten geschrieben. Die verwendeten Quellen stehen unter jedem Beitrag. Bei der Recherche und beim Schreiben kommen KI-gestützte Werkzeuge zum Einsatz; Zahlen werden gegen die genannten Quellen abgeglichen, und jeder Beitrag wird vor der Veröffentlichung vom Betreiber gesichtet.</li>
          <li><strong>Marktberichte und Kursnotizen</strong> werden automatisch aus offiziellen Schlusskursen erzeugt und sind als solche gekennzeichnet („automatisch aus Kursdaten erstellt“). Sie enthalten keine Einschätzungen, die nicht aus den Daten folgen.</li>
          <li><strong>Technische Analysen</strong> beschreiben Trend, Widerstände und Unterstützungen nach festen, in der <a href="/methodik">Methodik</a> erklärten Regeln. Sie sind keine Kauf- oder Verkaufsempfehlungen.</li>
          <li><strong>Gesponserte Beiträge</strong> („Im Fokus“) sind bezahlte Werbung. Sie tragen an jeder Stelle die Kennzeichnung „Anzeige“, nennen den Auftraggeber und bleiben von der redaktionellen Berichterstattung getrennt.</li>
        </ul>
        <h2>Korrekturen</h2>
        <p>Fehler passieren. Wenn Ihnen eine falsche Zahl, ein falscher Zusammenhang oder ein toter Link auffällt, schreiben Sie uns über die <a href="/kontakt">Kontaktseite</a>. Korrekturen werden im Beitrag kenntlich gemacht.</p>
        <h2>Regeln</h2>
        <p>Unabhängigkeit, Umgang mit Quellen und eigenen Wertpapiergeschäften: <a href="/redaktionelle-leitlinien">Redaktionelle Leitlinien</a>. Herkunft und Verzögerung der Kurse: <a href="/methodik">Methodik & Datenquellen</a>.</p>
      </div><aside>${c.sideCard('Mehr über uns', html`<ul class="side-list">${[['Über Börsenblick', '/ueber-uns'], ['Redaktionelle Leitlinien', '/redaktionelle-leitlinien'], ['Methodik & Datenquellen', '/methodik'], ['Werben', '/werben'], ['Kontakt', '/kontakt']].map(([l, h]) => html`<li><a href="${h}">${l}</a></li>`)}</ul>`)}</aside></div>
    </div>`;
    add('/redaktion', 'Redaktion', 'Wer hinter den Beiträgen steht, wie sie entstehen und wie Sie Korrekturen melden.', body);
  }
